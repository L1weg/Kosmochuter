...
def show_start_screen(): #!!! создаем меню
   menu = pygame_menu.Menu('Shoter', 300, 400, theme=pygame_menu.themes.THEME_BLUE) #!!!запускается меню с размером 300, 400
   menu.add.button('Начать', main) #!!! добавляем кнопку
   menu.add.button('Выйти', pygame_menu.events.EXIT) #!!! добавляем кнопку
   menu.mainloop(screen) #!!!запускаем главный цикл нашего экрана
